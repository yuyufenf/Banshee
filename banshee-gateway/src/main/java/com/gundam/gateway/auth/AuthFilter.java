package com.gundam.gateway.auth;

import com.alibaba.fastjson.JSONObject;
import com.gundam.commons.entity.UserLoginInfo;
import com.gundam.commons.utils.JwtUtil;
...
/**
 * 权限过滤器，进行鉴权，操作日志同步到数据库
 * @author kampf
 * @date 2020/4/11 上午9:04
 */
@Slf4j
@Component
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    /** 配置API请求密钥*/
    private static final String API_SECRET = "api";
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        operation.setVisitorService(gatewayUrl.getId());
        operation.setVisitorUrl(url);
        operation.setVisitorMethod(method);
        operation.setRemoteAddress(remoteAddress);

        // 异步执行日志存储
        operationService.addOperationLog(operation);
        // 将现在的request的请求头中，添加当前用户名称和API请求密钥，将其传递到下层服务中
        Consumer<HttpHeaders> httpHeaders = httpHeader -> {
            httpHeader.set("userName", userName);
            httpHeader.set("API_SECRET", API_SECRET);
        };
